...
def check_otel_collector_metrics():
    ...
    try:
        ...
        print(
            f"✗ Could not connect to OpenTelemetry Collector at {OTEL_COLLECTOR_URL}")
    ...
    if data and data.get("result"):
        ...
        if results:
            ...
            for result in results:
                ...
                print(
                    f"  User: {user_ip:15} Route: {route:10} Method: {method:6} Hits: {value}")
        else:
    ...
    if data and data.get("result"):
        ...
        if results:
            ...
            for result in results:
                ...
                total += value
                print(
                    f"  Route: {route:10} Method: {method:6} Total Hits: {value:.0f}")
            print(
    ...
    if data and data.get("result"):
        ...
        if results:
            ...
            for result in results:
                ...
                print(
                    f"  User: {user_ip:15} Route: {route:10} Type: {timeout_type:20} Count: {value}")
        else:
    ...
    if data and data.get("result"):
        ...
        if results:
            ...
            for result in results:
                ...
                total += value
                print(
                    f"  Route: {route:10} Type: {timeout_type:20} Total: {value:.0f}")
            print(
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("status") == "success":
                ...
                otel_target = [
                    t for t in targets if "otel-collector" in t.get("labels", {}).get("job", "")]
                ...
                if otel_target:
                    ...
                    if health == "up":
                        ...
                        print(
                            f"⚠ OpenTelemetry Collector target is {health.upper()}")
                        return False
                else:
...
def main():
    ...
    print("=" * 70)
    print(
        f"OpenTelemetry Collector: {'✓ OK' if otel_ok else '✗ Not accessible'}")
    print(
        f"Prometheus Targets: {'✓ OK' if targets_ok else '✗ Not accessible'}")
    print("\nAvailable Metrics:")
    print("  Per-user:")
